...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should display user's posts when not logged in" do
    ...
    users(:michelle).statuses.create(content: 'Non-blocked status')
    get :index
 ...
 test "should render the new page when logged in" do
  ...
  test "should create status for the current user when logged in" do
    sign_in users(:griffin)
     ...
     assert_difference('Status.count') do
      post :create, status: { content: @status.content, user_id: (users(:griffin).id) }
    end

    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:griffin).id
  end
...
test "should update status for the current user when logged in" do
   sign_in users(:griffin)
   put :update, id: @status, status: { content: @status.content, user_id: users(:michelle).id }
   assert_redirected_to status_path(assigns(:status))
   assert_equal assigns(:status).user_id, users(:griffin).id
  end
