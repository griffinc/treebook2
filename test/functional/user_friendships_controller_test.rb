...
class UserFriendshipsControllerTest < ActionController::TestCase
	context "#index" do
		...
		context "when logged in" do
			setup do
				@friendship1 = create(:pending_user_friendship, user: users(:griffin), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
				@friendship2 = create(:accepted_user_friendship, user: users(:griffin), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
				@friendship3 = create(:requested_user_friendship, user: users(:griffin), friend: create(:user, first_name: 'Requested', last_name: 'Friend'))
				@friendship4 = user_friendships(:blocked_by_griffin)

				sign_in users(:griffin)
				get :index	
			end
	...
	context "when logged in" do
		...
		should "should set a flash error if the friend_id is missions" do
			get :new, {}
			assert_equal "Friend required", flash[:error]
		end

		should "display the friend's name" do
			get :new, friend_id: users(:michelle)
			assert_match /#{users(:michelle).full_name}/, response.body
		end
		...
		get :new, friend_id: users(:michelle)
		assert_equal users(:michelle), assigns(:user_friendship).friend
		end

		should "assign a new user friendship to the currently logged in user" do
		get :new, friend_id: users(:michelle)
		assert_equal users(:griffin), assigns(:user_friendship).user
		end
		...
		should "ask if you really want to friend the user" do
			get :new, friend_id: users(:michelle)
			assert_match /Do you really want to friend #{users(:michelle).full_name}?/, response.body
		end
	end
end
		...
		context "successfully" do
			should "create two user friendship objects" do
				assert_difference 'UserFriendship.count', 2 do
					post :create, user_friendship: { friend_id: users(:griffin).profile_name }
				end
			end
		end
		...
		context "with a valid friend_id" do
			setup do
				post :create, user_friendship: { friend_id: users(:michelle) }
			end

			should "assign a friend object" do
				assert assigns(:friend)
				assert_equal users(:michelle), assigns(:friend)
			end
			...
			should "redirect to the profile page of the friend" do
				assert_response :redirect
				assert_redirected_to profile_path(users(:michelle))
			end
	...
	context "#accept" do
		...
		context "when logged in" do
			setup do
				@friend = create(:user)
				@user_friendship = create(:pending_user_friendship, user: users(:griffin), friend: @friend)
				create(:pending_user_friendship, friend: users(:griffin), user: @friend)
				sign_in users(:griffin)
	...
	context "when logged in" do
		setup do
			@user_friendship = create(:pending_user_friendship, user: users(:griffin))
			sign_in users(:griffin)
	...
	context "#detroy" do
		...
		context "when logged in" do
			setup do
				@friend = create(:user)
				@user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:griffin))
				create(:accepted_user_friendship, friend: users(:griffin), user: @friend)
		...
		context "#block" do
			...
			context "when logged in" do
				...
				should "update the user friendship state to be blocked" do
