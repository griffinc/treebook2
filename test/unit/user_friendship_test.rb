...
class UserFriendshipTest < ActiveSupport::TestCase
	...
	context "a new instance" do
		setup do
			@user_friendship = UserFriendship.new user: users(:griffin), friend: users(:michelle)
		end
	...
	context "#send_request_email" do
		setup do
			@user_friendship = UserFriendship.create user: users(:griffin), friend: users(:michelle)
		end
	...
	context "#mutual_friendship" do
		setup do
			UserFriendship.request users(:griffin), users(:michelle)
			@friendship1 = users(:griffin).user_friendships.where(friend_id: users(:michelle).id).first
 			@friendship2 = users(:michelle).user_friendships.where(friend_id: users(:griffin).id).first
		end
	...
	context "#accept!" do
		setup do
			@user_friendship = UserFriendship.request users(:griffin), users(:michelle)
		end
	...
	context "#block!" do
		...
		should "not allow new requests once blocked" do
			@user_friendship.block!
			uf = UserFriendship.request users(:griffin), users(:michelle)
			assert !uf.save
		end
	end
end
